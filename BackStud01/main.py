...
async def get_orders(limit: int = 20, offset: int = 0):
    ...
    conn = get_db_connection_dict()  # ← ИСПОЛЬЗУЕМ RealDictCursor
    cur = conn.cursor()
...
async def create_order(order: OrderCreate):
    ...
    schema = get_schema_prefix()
    conn = get_db_connection()  # ← ОБЫЧНОЕ ПОДКЛЮЧЕНИЕ (для INSERT)
    cur = conn.cursor()
    ...
    # Получаем имена клиента и водителя (используем RealDictCursor для чтения)
...
async def update_order_status(order_id: int, status_update: OrderUpdateStatus):
    ...
    # Проверяем, что заявка существует (используем RealDictCursor для чтения)
    conn_dict = get_db_connection_dict()
    cur_dict = conn_dict.cursor()
    ...
    # Обновляем статус (используем обычное подключение для UPDATE)
    ...
    # Получаем имена (используем RealDictCursor для чтения)
    conn_dict = get_db_connection_dict()
    cur_dict = conn_dict.cursor()
    cur_dict.execute(f"""
        SELECT 
